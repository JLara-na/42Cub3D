...
/*   Updated: 2024/10/25 20:58:00 by jlara-na         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_floor_ceiling(t_cam *cam, t_mlx *mlx, t_ray *ray)
{
	...
	while (++y < W_Y)
	{
		...
		ray->tx = (cam->x) * 1.1
			+ (cos(ray->ra) * (200 * 200)) / ray->dy / ray->rafix;
		ray->ty = (cam->y) * 1.1
			+ (sin(ray->ra) * (200 * 200)) / ray->dy / ray->rafix;
		ray->color = get_pixel_img(&mlx->floor, ((int)(ray->tx) & 63),
				((int)(ray->ty) & 63));
		my_mlx_pixel_put(&mlx->img, ray->i, y, ray->color);
		ray->color = get_pixel_img(&mlx->ceiling, ((int)(ray->tx) & 63),
				((int)(ray->ty) & 63));
