...
/*   Updated: 2024/10/12 17:08:51 by jlara-na         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	select_wall_color(t_mlx *mlx, t_ray *ray, int xau, int yau)
{
	...
	else
	{
		if (ray->ra < PI2 || ray->ra > _3PI2)
			ray->color = get_pixel_img(&mlx->img_wall[EAST],
					ray->tx, ray->ty);
		else
			ray->color = get_pixel_img(&mlx->img_wall[WEST],
					ray->tx, ray->ty);
	}
	...
	while (++yau < ray->lineh)
	{
		...
		while (++xau < (W_X) / RAY_NUMBER)
			if (get_map(cam, ray->rx / 64, ray->ry / 64) == '1')
				select_wall_color(mlx, ray, xau, yau);
		ray->ty += ray->ty_step;
	}
}
